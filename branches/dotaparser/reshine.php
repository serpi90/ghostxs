...
class replay {
  ...
  var $preAnnouncePick = array();
  // Store skills set before 0:15
  var $preAnnounceSkill = array();
...
function loadgame() {
    ...
    $map_file = getMapName($this->game['map'], $this->game['dota_major'], $this->game['dota_minor']);
    if ( $map_file === false ) {
        return "fail";
    }
     
    $GLOBALS['xml_data']->parse_file(MAPS_FOLDER."/".$map_file);
    ...
    for ($i=0; $i<$slot_records; $i++) {
        ...
        $temp['dota_id'] = getDotaId($temp['color']);   /* Seven */
        // Used for handling SP mode
        $this->dotaIdToWc3id[getDotaId($temp['color'])] = $temp['player_id'];
    ...
    while ($n < $block_length) {
      ...
      switch ($action) {
        ...
        case 0x10:
          ...
          if (!$value) {
            if(isset($this->players[$player_id]['actions_details'][convert_action('ability')])) {
            	$this->players[$player_id]['actions_details'][convert_action('ability')]++;
            } else { 
	            $this->players[$player_id]['actions_details'][convert_action('ability')] = 1; 
            }
            ...
            if (ord($actionblock{$n+2}) == 0x33 && ord($actionblock{$n+3}) == 0x02) {
	          ...
	          if(isset($this->players[$player_id]['actions_details'][convert_action('buildtrain')])) {
	          	$this->players[$player_id]['actions_details'][convert_action('buildtrain')]++;
	          } 
              else { 
	            $this->players[$player_id]['actions_details'][convert_action('buildtrain')] = 1; 
	          }
            ...
            switch ($type) {
                case 'HERO':
                      ...
                      if($this->inPickMode) {
                            ...
                            if($this->picks_num >= NUM_OF_PICKS) {
                                $this->pause = false;
                                $this->inPickMode = false;
                            }
                            
                            if(DEBUG_ON) {
                                echo "Added hero to pool in CM mode ".$value->getName()." ID ".$value->getId()." for player ".$this->players[$player_id]['name']." at ".convert_time($this->time)."<br />"; 
                            }
                ...
                case 'STAT':
                  ...
                  // If skill-to-hero is the same as player's hero or common attribute skill the hero
                      ...
                      if($this->stats[$pid]->getLevelCap() >= 2) {
                          ...
                          if( DEBUG_ON ) {
                            echo "Player ".$this->players[$player_id]['name']." skilling ".$heroName." at ".$this->time." (PLAYER ID : ".$player_id.")<br />";
                          }
                      }          
                  ...
                  // Otherwise assume the player's skilling a Hero not owned by him
                      ...
                      if ( DEBUG_ON ) {
                        echo "Player ".$this->players[$player_id]['name']." skilling non-owned ".$heroName." at ".$this->time." (PLAYER ID : ".$player_id.") - DOTA ID: ".$this->players[$player_id]['dota_id']." Translated: ".$this->translatedDotaID[$this->players[$player_id]['dota_id']]." <br />";
                      }
                  } 
                ...
                case 'ITEM':
                  $this->players[$player_id]['items'][convert_time($this->time)] = $value;
                  break;
...
// echo $x.'<br />'.$y.'<br /><br />';
                ...
                if (ord($actionblock{$n+2}) <= 0x19 && ord($actionblock{$n+3}) == 0x00) { // basic commands
                  if(isset($this->players[$player_id]['actions_details'][convert_action('basic')])) {
              	    $this->players[$player_id]['actions_details'][convert_action('basic')]++;
                  } 
                  else { 
	                  $this->players[$player_id]['actions_details'][convert_action('basic')] = 1; 
                  }
                } 
                else {
                  if(isset($this->players[$player_id]['actions_details'][convert_action('ability')])) {
          ...
          case 0x12:
            ...
            if (ord($actionblock{$n+2}) == 0x03 && ord($actionblock{$n+3}) == 0x00) { // rightclick
              if(isset($this->players[$player_id]['actions_details'][convert_action('rightclick')])) {
              	$this->players[$player_id]['actions_details'][convert_action('rightclick')]++;
              } 
              else { 
	              $this->players[$player_id]['actions_details'][convert_action('rightclick')] = 1; 
              }
            } 
          ...
          case 0x13:
            ...
            if(isset($this->players[$player_id]['actions_details'][convert_action('item')])) {
            	$this->players[$player_id]['actions_details'][convert_action('item')]++;
            } else { 
	            $this->players[$player_id]['actions_details'][convert_action('item')] = 1; 
            }
            $n+=38;
            break;
          ...
          case 0x16:
            ...
            if ($temp['mode'] == 0x02 || !$was_deselect) {
                $this->players[$player_id]['actions'][] = $this->time;
                if(isset($this->players[$player_id]['actions_details'][convert_action('select')])) {
            	    $this->players[$player_id]['actions_details'][convert_action('select')]++;
                } 
                else { 
	                $this->players[$player_id]['actions_details'][convert_action('select')] = 1; 
                }
            }
          ...
          case 0x17:
            $this->players[$player_id]['actions'][] = $this->time;
            if(isset($this->players[$player_id]['actions_details'][convert_action('assignhotkey')])) {
            	$this->players[$player_id]['actions_details'][convert_action('assignhotkey')]++;
            } 
            else { 
	            $this->players[$player_id]['actions_details'][convert_action('assignhotkey')] = 1; 
            }
          ...
          case 0x18:
            $this->players[$player_id]['actions'][] = $this->time;
            if(isset($this->players[$player_id]['actions_details'][convert_action('selecthotkey')])) {
            	$this->players[$player_id]['actions_details'][convert_action('selecthotkey')]++;
            } 
            else { 
	            $this->players[$player_id]['actions_details'][convert_action('selecthotkey')] = 1; 
            }
          ...
          case 0x19:
            ...
            if ($this->header['build_v'] >= 6040 || $this->header['major_v'] > 14) {
              if ($was_subgroup) { // can't think of anything better (check action 0x1A)
                $this->players[$player_id]['actions'][] = $this->time;
                if(isset($this->players[$player_id]['actions_details'][convert_action('subgroup')])) {
                	$this->players[$player_id]['actions_details'][convert_action('subgroup')]++;
                } else { 
	                $this->players[$player_id]['actions_details'][convert_action('subgroup')] = 1; 
                }
          ...
          case 0x1E:
            ...
            if (($this->header['build_v'] >= 6040 || $this->header['major_v'] > 14) && $action != 0x1E) {
              ...
              $this->players[$player_id]['actions'][] = $this->time;
              if(isset($this->players[$player_id]['actions_details'][convert_action('removeunit')])) {
              	$this->players[$player_id]['actions_details'][convert_action('removeunit')]++;
              } else { 
	              $this->players[$player_id]['actions_details'][convert_action('removeunit')] = 1; 
              }
          ...
          case 0x60:
            ...
            $n+=1;
            if ( DEBUG_ON ) {
                echo "Trigger chat command: ".$str." at ".convert_time($this->time)."<br />";
            }
            break;
          ...
          case 0x61:
            $this->players[$player_id]['actions'][] = $this->time;
            if(isset($this->players[$player_id]['actions_details'][convert_action('esc')])) {
            	$this->players[$player_id]['actions_details'][convert_action('esc')]++;
            } 
            else { 
	            $this->players[$player_id]['actions_details'][convert_action('esc')] = 1; 
            }
            $n+=1;
            break;
          ...
          case 0x65:
            $this->players[$player_id]['actions'][] = $this->time;
            if(isset($this->players[$player_id]['actions_details'][convert_action('heromenu')])) {
            	$this->players[$player_id]['actions_details'][convert_action('heromenu')]++;
            } else { 
	            $this->players[$player_id]['actions_details'][convert_action('heromenu')] = 1; 
            }
            $n+=1;
            break;
          ...
          case 0x67:
            if ($this->header['major_v'] >= 7) {
              $this->players[$player_id]['actions'][] = $this->time;
              if(isset($this->players[$player_id]['actions_details'][convert_action('buildmenu')])) {
              	$this->players[$player_id]['actions_details'][convert_action('buildmenu')]++;
              } 
              else { 
	              $this->players[$player_id]['actions_details'][convert_action('buildmenu')] = 1; 
              }
              $n+=1;
            } else {
              $n+=13;
 ...
 */
          ...
          case 0x6B:
            ...
            if($MissionKey == "Data" ) {
                ...
                */
                ...
                if( strstr($Key, "Mode") !== false ) {
                    ...
                    switch ( $shortMode ) {
                        case "cd":
                            $this->dotaMode = new ModeCD();
                            break;
                        case "cm":
                            $this->dotaMode = new ModeCM();
                            break;
                    }
                }
            ...
            if(DEBUG_ON) {
                echo "<hr />Debug time: ".convert_time($this->time)." <br />";
 ...
 */        
  ...
  function cleanup() {
	...
	foreach ($this->players as $player) {
      ...
      $baseGameDotaId = getGameDotaId($intDotaId);
