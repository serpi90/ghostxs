...
define("XML_MAP_BASE_NAME", "dota.allstars.v");
...
define("DUPLICATE_SKILLING_TIME_LIMIT", 200);
...
*/
function getMapName($mapName, &$dota_major, &$dota_minor) {
    ...
    if( count($matches) < 4 ) {
        ...
        if ( file_exists(MAPS_FOLDER."/".XML_MAP_BASE_NAME.$dota_major.".".$dota_minor.$dota_subver.".xml") ) {
            // Use it 
            return XML_MAP_BASE_NAME.$dota_major.".".$dota_minor.$dota_subver.".xml";  
        }
        // Check if an appropriate .xml file without subversion exists  ( ie. dota.allstars.v6.60.xml )
        else if ( file_exists(MAPS_FOLDER."/".XML_MAP_BASE_NAME.$dota_major.".".$dota_minor.".xml") ) {
             // Use it
             return XML_MAP_BASE_NAME.$dota_major.".".$dota_minor.".xml";   
        }
...
class PlayerStats {
  ...
  */
  function getGameDotaId($internalDotaID) {
...
function convert_itemid($value) {
    ...
    // When DEBUG_ON_TOOLS is turned on true, we can capture unknown ID's
    if( DEBUG_ON_TOOLS ) { 
...
function convert_chat_mode($value) {
  switch ($value) {
